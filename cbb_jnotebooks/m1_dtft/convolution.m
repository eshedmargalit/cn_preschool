...
close all; clear all;
%% conv basics
	% establish square wave
	wv = ones(1, 50);
	wv(20:30) = 2;
	k = [2 1 0 -1 -2]; % in wavelet analysis later we will have kernels with zero mean
	k = [5 4 3 2 1];
	% convolution, our own is in the "play through" section
	conv_k_wv = conv(k, wv);
	
	% establish static figure
	fh = figure();
	...
	axis(sh(2), [0 numel(wv) min(k) max(k)]);
        % check if you understand, how long is the final product?

%% play through
	% convolution raw materials 
	pad = numel(k)-1;
	zpad_wv = [zeros(1, pad), wv, zeros(1, pad)];%pad wave
	% establish flick figure
	fh2 = figure();
	...
	% write our own convolution
	conv_idx = [1:numel(wv) + pad] + floor(numel(k)/2);%places dot product in correct position
	for idx = 1:numel(wv)+pad
		% kernel is wrong way on purpose
		%fl_k = fliplr(k); % the right way to do convolution
		fl_k = k; % this is a correlation
		fl_k_position = idx:idx+pad; % center of kernel
		...
		lh2(3) = line(conv_idx(1:numel(man_conv)), man_conv, 'color', 'k',...
		 'parent', sh2(3));
		pause(1)
		delete(lh2(2)); % this is the kernel
